    ...
    def register_sparse_param(self):
        ...
        for n, p in self.model.named_parameters():
            ...
            if any([m in n for m in self.sparse_module]):
                ...
                sparse_idx = torch.tensor(random.sample(list(range(p.numel())), train_num), dtype=torch.int64) 
                # sparse_param.idx = np.stack(np.unravel_index(sparse_idx, p.shape))
                sparse_param.idx = torch.stack(torch.unravel_index(sparse_idx, p.shape))
                    ...
                    if n in self.sparse_mapping.keys():
                        if p.grad is not None :
                            ...
                            if not self.if_get_idx[n]:
                                ...
                                sparse_idx = torch.flatten(abs(grad)).topk(sparse_param.train_num).indices 
                                # sparse_param.idx = np.stack(np.unravel_index(sparse_idx, p.shape))
                                sparse_param.idx = torch.stack(torch.unravel_index(sparse_idx, p.shape))
                            ...
                            if sparse_param.grad != None:
                                sparse_param.grad += grad[tuple(sparse_param.idx)]
                            else:
                                sparse_param.grad = grad[tuple(sparse_param.idx)]
