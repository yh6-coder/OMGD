...
class OurArguments(TrainingArguments):
    ...
    train_all: bool = False  # train on all training samples and evaluate on the test set
    ...
    dampening: float = 0.0  # only work for SGD optimizer
...
class Framework:
    ...
    def load_model(self):
        ...
        """
        with count_time("Loading model with %s%d" % (("BF" if self.args.load_bfloat16 else "FP"), (16 if self.args.load_float16 or self.args.load_bfloat16 else 32))):
        ...
        if self.args.lora:
            with torch.cuda.amp.autocast(enabled=self.args.load_bfloat16):
...
def main():
    ...
    args.tag = f"{args.trainer}-{args.task_name}-{args.template_ver}-{args.model_name.split('/')[-1]}-OPTIM_{args.mode}-EP{args.num_train_epochs}-{args.optimizer}-LR{args.learning_rate}-{args.lr_scheduler_type}-{datetime.now().strftime('%Y%m%d%H%M%S')}" # time
    ...
    if args.train_all:
        args.num_train = int(len(task.samples["train"]) * 0.9)
        args.num_dev = len(task.samples["train"]) - args.num_train
        args.num_eval = len(task.samples["valid"])
    ...
    with torch.cuda.amp.autocast(enabled=args.load_bfloat16):
