...
def parse_args():
    ...
    parser.add_argument("--enable_golore", action="store_true", help="Whether or not to use low rank optimizer.")
    parser.add_argument("--rand_ratio", type=float, default=2)

    # modifyed args
    parser.add_argument('--sampling_layers', type=int, default=4, help='Number of layers to sample per epoch (for layer-wise mask without replacement)')
    parser.add_argument('--sampling_period', type=int, default=2, help='Number of epochs between sampling new layers (for layer-wise mask without replacement)')
    parser.add_argument("--enable_sift", action="store_true", help="Whether or not to use low rank optimizer.")
    parser.add_argument("--enable_lisa", action="store_true", help="Whether or not to use low rank optimizer.")
    parser.add_argument("--enable_lisa_wor", action="store_true", help="Whether or not to use low rank optimizer.")
...
def main():
    ...
    accelerator.init_trackers("GaCoRe_FT", run_config)
    if args.with_tracking:
        accelerator.get_tracker('wandb', unwrap=True).save(os.path.abspath(__file__), policy="now") # save current script
        accelerator.get_tracker('wandb', unwrap=True).save('./peft_pretraining/*', policy="now") # save current script
        accelerator.get_tracker('wandb', unwrap=True).save('./peft_pretraining/galore_torch/*', policy="now") # save current script
        # accelerator.get_tracker('wandb', unwrap=True).save('./peft_pretraining/galore_torch_sgd/*', policy="now") # save current script
...
################################################################################################################
    ...
    from peft_pretraining.GoLore import ReLoRaModel
    if args.enable_golore:
        model = ReLoRaModel(
            model,
            r=args.lora_r,
            lora_dropout=0,
    ...
    if args.enable_golore:
        ...
        from peft_pretraining.golore_torch import GoLoreAdamW
        optimizer = GoLoreAdamW(optimizer_grouped_parameters, lr=args.learning_rate)
    elif args.enable_galore:
        from torch import nn
        from peft_pretraining.galore_torch import GaLoreAdamW
        galore_params = []
    ...
    elif args.enable_sift:
        from sift import SIFT
        sift = SIFT(model, sparse_rate=0.1, sparse_module=target_modules_list,
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
            ...
            if not hasattr(args, 'layer_wise_masks') or (epoch % args.sampling_period == 0):
                ...
                for idx, (name, _) in enumerate(named_params):
                    ...
                    if name.startswith('roberta.encoder.layer'):
                        layer_num = int(name.split('.')[3])  # Extract layer number, e.g., '0', '11'
                        # if rank in [-1, 0]:print(layer_num)
                        if layer_num not in layer_groups:
                            layer_groups[layer_num] = []
                        layer_groups[layer_num].append(idx)
                # if rank in [-1, 0]:print(layer_groups)
                middle_layers = list(layer_groups.keys())
                # Randomly select args.sampling_layers layers to activate
                selected_indices = torch.randperm(len(middle_layers))[:args.sampling_layers]
                mask = torch.zeros(len(list(model.parameters())), dtype=torch.bool)
                ...
                for idx, (name, _) in enumerate(named_params):
                    if not name.startswith('roberta.encoder.layer'):
                        # if rank in [-1, 0]: print(f"Always True param: {name}")
                        mask[idx] = True
                args.layer_wise_masks = [mask]
                if accelerator.is_main_process:
                    print(middle_layers)
                    print(selected_indices)
        if args.enable_lisa_wor:
        ...
        if args.enable_lisa or args.enable_lisa_wor:
            mask = args.layer_wise_masks[0]
            for j, param in enumerate(list(model.parameters())):
                param.requires_grad = mask[j].item()
        for step, batch in enumerate(active_dataloader):
            if isinstance(model, ReLoRaModel):
                reset_relora = completed_steps % args.update_proj_gap == 0
            ...
            if args.enable_lisa_wor:
                ...
                for j, param in enumerate(list(model.parameters())):
                    ...
                    if mask[j].item() and param.grad is not None and name.startswith('roberta.encoder.layer'):
                        # if rank in [-1, 0]: print(name)
                        param.grad.mul_(12/args.sampling_layers)  # Scale factor
            if step % args.gradient_accumulation_steps == 0 or step == len(train_dataloader) - 1:
                ...
                if args.with_tracking:
                    accelerator.log(
                        {
                            "mid_loss": tot_loss1 / local_steps
                        },
        ...
        # Record evaluation for this epoch
        all_results[f"epoch_{epoch}"] = eval_metric

        # Compute max metrics across epochs so far
        metric_names = set()
        for key in all_results:
            if key.startswith("epoch_"):
                metric_names.update(all_results[key].keys())
        for metric_name in metric_names:
            max_val = max(all_results[key][metric_name] for key in all_results if key.startswith("epoch_") and metric_name in all_results[key])
            all_results[f"max_{metric_name}"] = max_val

        # Write to JSON file after each epoch
        ...
        if args.push_to_hub and epoch < args.num_train_epochs - 1:
            ...
            if isinstance(model, ReLoRaModel): unwrapped_model = model.wrapped_model
    ...
    if args.task_name == "mnli":
        ...
        # Record mnli-mm evaluation
        all_results["mnli-mm"] = eval_metric

    if args.output_dir is not None:
        # Compute max metrics across epochs
        metric_names = set()
        for key in all_results:
            if key.startswith("epoch_"):
                metric_names.update(all_results[key].keys())
